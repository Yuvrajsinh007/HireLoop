...
const StatsCard = ({ title, value, icon, color = "indigo", subtitle = "", trend = null }) => {
  const colorMap = {
    ...
    green:   { bg: "bg-emerald-50",  text: "text-emerald-600", border: "border-emerald-100", icon: "text-emerald-600" },
    yellow:  { bg: "bg-amber-50",    text: "text-amber-600",   border: "border-amber-100",   icon: "text-amber-600" },
  };
  ...
  const renderIcon = () => {
    if (!icon) return null;
    // Already a rendered JSX element, e.g. icon={<Globe className="..." />}
    if (isValidElement(icon)) return icon;
    // Emoji or plain string
    if (typeof icon === "string") return <span className="text-2xl">{icon}</span>;
    // A component reference, e.g. icon={Globe}
    const Icon = icon;
    return <Icon className={`w-6 h-6 ${c.icon} stroke-[1.5]`} />;
  };
